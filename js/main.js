...
function getQuestions() {
  ...
  myRequest.open("GET", "/json/questions.json");
  myRequest.send();
}
...
function addQuestionsData(obj, count) {
  if (currentIndex < count) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      console.log("obj[`answer_${i + 1}`: ", obj[`answer_${i + 1}`]);
...
function showREsults(count) {
  ...
  if (currentIndex === count) {
    ...
    if (rightAnswer >= count / 2 && rightAnswer < count) {
      theResulted = `<span class="good"> Good  </span> ${rightAnswer}/${count} you can get better `;
    } else if (rightAnswer === count) {
      theResulted = `<span class="perfect"> Perfect  </span> ${rightAnswer}/${count}<img src="/png/kisspng-check-mark-clip-art-green-tick-png-free-download-5a75700e2f8582.9282309115176458381947.png" alt="">`;
    } else {
      theResulted = `<span class="bad"> Bad  </span> ${rightAnswer}/${count} learn and try again later  `;
    }
